...
import type { UserDashboardProfileData } from "@/interfaces/User_interface";
import { getUserProfileData, UpdateUserDashProfile } from "@/api/Users_api";

export default function UpdateUserProfile() {
  const { user } = useAuth();
  const [profile, setProfile] = useState<UserDashboardProfileData>({
    user_id: user?.userId || "",
    name: "",
    email: "",
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        ...
        try {
            setLoading(true);
            const response = await getUserProfileData(user.userId, user.token); 
            setProfile(response);
        } catch (error: any) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      setLoading(true);
      const response =await UpdateUserDashProfile(profile, user?.email, user?.token);
      setProfile(response);
