...
import { resetUserPassword } from "@/api/Users_api";
import type { ResetUserPasswordDto } from "@/interfaces/User_interface";

const passwordRegex =
  /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_\-+=\[{\]};:'",<.>/?\\|`~]).{8,16}$/;

export default function ResetUserPassword() {
  const [formData, setFormData] = useState<ResetUserPasswordDto>({
    user_id: "",
    password: "",
  });
  ...
  const handleResetPassword = async () => {
    ...
    try {
      setLoading(true);
      const response = await resetUserPassword(formData, user?.token);
      if(response.success){
