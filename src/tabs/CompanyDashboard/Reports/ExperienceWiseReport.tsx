import { getGetExperienceWiseProfiles } from "@/api/Reports_api";
...
export interface ExperienceWiseDataReq {
  jobId: number;
  skillId: number[];
  minExp: number;
}
...
export default function ExperienceWiseReport() {
  const { user } = useAuth();
  ...
  const [args, setArgs] = useState<ExperienceWiseDataReq>({
    jobId: 0,
    skillId: [],
    minExp: 0,
  });
  ...
  useEffect(() => {
    ...
    const storedArgs = sessionStorage.getItem("ExpWiseArgs");
    if (storedArgs) {
      const parsed = JSON.parse(storedArgs) as ExperienceWiseDataReq;
      setArgs(parsed);
    }
  }, []);
  ...
  const handleGenerate = () => {
    const payload: ExperienceWiseDataReq = {
      jobId: args.jobId,
    ...
    setArgs(payload);
    sessionStorage.setItem("ExpWiseArgs", JSON.stringify(payload));
  };
      ...
      <CardContent>
        <div>
          <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="flex flex-row gap-4">
                ...
                {args?.jobId !== 0 && (
                  <div className="flex items-center justify-center border p-1 px-3 rounded-md">
                    {
                      jobs?.find((item) => item.job_id === args.jobId)
                        ?.job_title
                    }
                  </div>
                )}
            ...
            <div className="">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">
                Select Minimum Experience {"( Years )"}:
              </h3>
              <Input
                type="number"
                className="w-[180px]"
        ...
        {args.skillId.length > 0 && (
          <ListManager<TechnologyWiseDataRes>
            fetchFunction={getGetExperienceWiseProfiles}
            addLink=""
            fetchArgs={[args]}
            columns={columns}
            addButton={false}
